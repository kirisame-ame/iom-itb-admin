              ...
              <table class="min-w-full leading-normal">
                ...
                <tbody>
                  ...
                  <tr v-else v-for="(u, index) in computedData" :key="index">
                    ...
                    >
                      <p class="text-gray-900 whitespace-nowrap">{{ u?.penerima?.nama }}</p>
                    </td>
                    <td
                      class="px-5 py-5 text-sm bg-white border-b border-gray-200"
                    >
                      <p class="text-gray-900 whitespace-nowrap">{{ u?.penerima?.nim }}</p>
                    </td>
                    <td
                      class="px-5 py-5 text-sm bg-white border-b border-gray-200"
                    >
                      <p class="text-gray-900 whitespace-nowrap">{{ u?.penerima?.program_studi }}</p>
                    </td>
                    <td
                      class="px-5 py-5 text-sm bg-white border-b border-gray-200"
                    >
                      <p class="text-gray-900 whitespace-nowrap">{{ u?.jenis_bantuan }}</p>
                    </td>
                    <td
                      class="px-5 py-5 text-sm bg-white border-b border-gray-200"
                    >
                      <p class="text-gray-900 whitespace-nowrap">{{ u?.bulan }}</p>
                    </td>
                    <td
                      class="px-5 py-5 text-sm bg-white border-b border-gray-200"
                    >
                      <p class="text-gray-900 whitespace-nowrap">{{ u?.tahun }}</p>
                    </td>
                    <td
                      class="px-5 py-5 text-sm bg-white border-b border-gray-200"
                    >
                      <p class="text-gray-900 whitespace-nowrap">{{ formattedPrice(u?.jumlah_donasi) }}</p>
                    </td>
                    <td
                      class="px-5 py-5 text-sm bg-white border-b border-gray-200"
                    >
...
import IcPlus from '@/assets/svg/ic-plus.vue';
  ...
  const computedData = computed(() => {
    const data = store.getters.danaBantuan; // Menggunakan store di sini
    return data.data || []; // Menghindari undefined
  });
  
  const pagination = computed(() => {
    const data = store.getters.danaBantuan; // Menggunakan store di sini
    return data?.pagination || []; // Menghindari undefined
  });
  ...
  const getData = async () => {
    ...
    const data = await store.dispatch(GET_DANA_BANTUAN, params);
     ...
     await Swal.fire({
      ...
      confirmButtonText: 'Yes, delete it!'
            ...
            if (result.isConfirmed) {
              const params = { id: id };
              await store.dispatch(DELETE_DANA_BANTUAN, params);
              try {
                Swal.fire({
                  title: "Deleted!",
