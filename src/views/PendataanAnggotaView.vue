          ...
          <tbody>
            <template v-if="isLoading">
              <tr v-for="i in limit" :key="i" class="border-b border-slate-100">
                <td v-for="c in columns.length + 3" :key="c" class="px-5 py-4">
            ...
            <template v-else-if="!rows.length">
              <tr>
                <td :colspan="columns.length + 3" class="px-5 py-12 text-center text-sm text-slate-400 italic">
              ...
              >
                ...
                <td class="px-5 py-4 text-center whitespace-nowrap">
                  <button
                    ...
                    :disabled="sendingWhatsappId === row.tallySubmissionId || !row.extractedWhatsapp"
                    class="inline-flex items-center justify-center gap-1.5 px-3.5 py-1.5 text-xs font-semibold text-white bg-emerald-600 rounded-full hover:bg-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm"
                    :title="row.extractedWhatsapp ? 'Kirim pesan WhatsApp' : 'Nomor WhatsApp tidak tersedia'"
                  >
                    <span v-if="sendingWhatsappId === row.tallySubmissionId" class="w-3.5 h-3.5 rounded-full border-2 border-white/40 border-t-white animate-spin" />
                    <svg v-else class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M8 10h.01M12 10h.01M16 10h.01M9 16H7a4 4 0 0 1-4-4V8a4 4 0 0 1 4-4h10a4 4 0 0 1 4 4v4a4 4 0 0 1-4 4h-3l-4 4v-4Z"/>
                    </svg>
                    {{ sendingWhatsappId === row.tallySubmissionId ? 'Mengirim' : 'Kirim WA' }}
                  </button>
                </td>
                <td
                  v-for="col in columns"
...
import { GET_PENDATAAN_ANGGOTA, SEND_WHATSAPP_PENDATAAN_ANGGOTA } from '@/store/pendataanAnggota.module'
...
const sendingWhatsappId = ref<string | null>(null)
...
async function sendWhatsapp(row: { tallySubmissionId: string; extractedWhatsapp?: string | null }) {
  if (!row.extractedWhatsapp) return

  const confirmation = await Swal.fire({
    title: 'Kirim WhatsApp?',
    text: `Kirim pesan WhatsApp ke ${row.extractedWhatsapp}?`,
    icon: 'question',
  ...
  sendingWhatsappId.value = row.tallySubmissionId
  try {
    await store.dispatch(`pendataanAnggota/${SEND_WHATSAPP_PENDATAAN_ANGGOTA}`, row.tallySubmissionId)
    await Swal.fire({
      icon: 'success',
    ...
    await Swal.fire({
      ...
      text: error instanceof Error ? error.message : 'Terjadi kesalahan saat mengirim pesan WhatsApp.',
      confirmButtonColor: '#2563eb',
    })
  } finally {
