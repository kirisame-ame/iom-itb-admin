<template>
  <div>
    <!-- Breadcrumb -->
    <Breadcrumb breadcrumb="Pengajuan Bantuan" />

    <!-- Modal -->
    <ModalForm
      v-if="isOpened"
      :id="currentId"
      :title="`${currentId? 'Edit' : 'Add'} Pengajuan Bantuan`"
      :data="dataUpdate"
    ...
    <div class="mt-8">
      ...
      <div class="mt-6">
        <h2 class="text-xl font-semibold leading-tight text-gray-700">Pengajuan Bantuan</h2>
              ...
              <tbody>
                <tr v-for="(u, index) in computedData" :key="index">
                    ...
                    <div class="flex items-center">
                      <div class="flex-shrink-0 w-10 h-10">
                        <img
                          ...
                          :src="u?.proof"
                          alt="profile pic"
                          @click="openImageModal(u?.toPropose)"
                        />
                        <img
                          v-else
                          class="w-[50px] h-[50px] rounded-[4px] cursor-pointer hover:opacity-[0.8]"
                  ...
                  <td
                    class="px-5 py-5 text-sm bg-white border-b border-gray-200"
                  >
                    <p class="text-gray-900 whitespace-nowrap">{{ u?.type }}</p>
                  </td>
                  <td
                    class="px-5 py-5 text-sm bg-white border-b border-gray-200"
                  >
                    <p class="text-gray-900 whitespace-nowrap">{{ u?.reason }}</p>
                  </td>
                  <td
                    class="px-5 py-5 text-sm bg-white border-b border-gray-200"
                  >
...
import { GET_HELP_SUBMISSIONS, DELETE_HELP_SUBMISSION } from "@/store/helpSubmission.module";
...
const computedData = computed(() => {
  const helpSubmissions = store.getters.helpSubmissions; // Menggunakan store di sini
  return helpSubmissions || []; // Menghindari undefined
});
...
const getData = async () => {
  ...
  const data = await store.dispatch(GET_HELP_SUBMISSIONS, params);
  return data;
};
    ...
    confirmButtonText: 'Yes, delete it!'
          ...
          if (result.isConfirmed) {
            ...
            try {
              ...
              await store.dispatch(DELETE_HELP_SUBMISSION, params);
            } catch (err) {
              console.log(err);
            }
          }
